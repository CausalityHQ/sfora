...
ASGCV_E0_CUSTODY_SCHEMA = "sfora-asgcv-e0-custody-v1"
...
def canonical_e0_custody_bytes(
    *,
    ...
    if (
        ...
        or len(sample_receipts) != ASGCV_E0_MINIMUM_PAIRS
    ):
        raise ValueError("ASG-CV E0 custody shape differs")
    arrays = result["arrays"]
    if type(arrays) is not dict or arrays["exact_gradients"] != _array_authority(
        exact,
    ...
    flat = exact.astype(np.float32).reshape(ASGCV_E0_MINIMUM_PAIRS, *exact.shape[2:])
