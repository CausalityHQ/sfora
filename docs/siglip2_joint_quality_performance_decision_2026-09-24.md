...
The only completed training-speed evidence is a synthetic-objective
three-step preflight on one NVIDIA GB10: at microbatch 64, 1.136–1.173
...
These are not end-to-end 1,000-update or data-pipeline throughput numbers.
...
receipt field; its zero-update gate remains pending. The first 1,000-update
control failed at step 131 from nonfinite
fp16 gradients. A 150-step single-variable replay with initial loss scale
128 relative to the instrumented v8 source passed, so the revised
1,000-update control is active. This is a
stability decision, not a quality win.

After the control completes, run float-rank and deployed-code-rank arms
