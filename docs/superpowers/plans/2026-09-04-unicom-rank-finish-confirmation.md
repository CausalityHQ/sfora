...
2. Add a strict confirmation evaluator that authenticates the seed-0 screen and
   seed-1/2 receipts, recomputes every per-seed delta and the three-seed mean,
   and emits a canonical pass/fail receipt.
...
5. Run the offline confirmation evaluator. If and only if it passes, execute one
   fixed seed-1 standard-test readout and publish the paired quality receipt.
