...
- Produces: CLI `run_pass209_m4_cell.py --cell {dinov2-large,siglip2-so400m,siglip-so400m} --prerequisite PATH --error-manifest PATH --receipt-output PATH --descriptor-output PATH --query-output PATH --source-revision REV --source-tree-digest SHA256 --uv-lock PATH --execute`.

- [x] **Step 1: Write a failing strict-CLI test.** Require all three new output paths, exact cell name, explicit `--execute`, offline environment, source revision/tree digest, prerequisite receipt, the legacy set digest plus new ordered example digest, historical batch 32 for DINOv2-L and batch 8 for both SigLIP cells, query block 32, and refusal of test/clean-class/model/layer/readout override flags.
...
- [x] **Step 4: Implement frozen fp32 encoding and reproduction.** Reuse the established RGB materializer and registered Transformers readouts, enforce processor shape, full fp32 tower/readout, unit normalization, deterministic CUDA, TF32 off, historical batch 32 for DINOv2-L and batch 8 for both SigLIP cells, then rerun the historical CUDA scorer (including its second normalization). Bind GPU product/UUID/compute capability, driver, CUDA runtime/build, and cuDNN. Emit batch-boundary progress receipts and record the exact historical aggregate/error-position reproduction gates.
- [x] **Step 5: Score on CPU and publish once.** Move only the final descriptor plane to CPU, release CUDA model/cache, invoke Task 2, build all three payloads in memory or owned partials, and atomically publish them together. A historical reproduction mismatch publishes a claim-ineligible failed receipt plus its completed descriptor/query evidence and exits nonzero; the analyzer refuses it.
- [x] **Step 6: Add fake-model integration tests.** Mutation-lock readout, revision, processor shape, count, batch ordering, resume identity, overwrite refusal, partial cleanup, and proof that no clean/test split is loaded.
- [x] **Step 7: Run the runner tests, Ruff, mypy, py_compile, and commit.**
