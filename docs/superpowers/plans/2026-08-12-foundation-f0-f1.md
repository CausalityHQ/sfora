...
- No model may load from a mutable tag: every executable arm requires an exact revision, weight digest, processor digest, pooling rule, resolution, dtype, and normalization rule.
...
- This plan does not implement Matryoshka adaptation, student distillation, compression controls, ILIAS encoding, or custom kernels.
...
**Interfaces:**
- Produces: `FoundationModelSpec`, `FoundationEncoderAudit`, `load_foundation_encoder(spec)`.

- [ ] **Step 1: Write failing registry and loader tests**

Create literal specs in the test for DINOv3-S, DINOv3 ConvNeXt-Tiny, and SigLIP2. Assert that `AutoImageProcessor.from_pretrained` and `AutoModel.from_pretrained` receive `revision=spec.revision`; reject empty revisions/digests and an observed config digest different from the registered digest.
...
def load_foundation_encoder(spec: FoundationModelSpec) -> FrozenImageEncoder:
...
Compute observed processor/config/weight-file digests before returning the encoder and fail closed on mismatch. Record gated/unavailable models as structured unavailable audit rows; never substitute a model.
...
Run: `pytest -q tests/test_image_benchmark.py -k recall_at_100 tests/test_foundation_pareto.py -k geometry`

- [ ] **Step 3: Implement the metric and evaluator**

Add `recall_at_100` to `ImageRetrievalMetrics`, include cutoff 100 in the shared scoring loops, and preserve existing cutoff fields. `evaluate_foundation_geometries` returns all three registered rows without selecting one.
...
**Interfaces:**
- Produces: `fit_bias_free_probe_512(...)`, `decide_f1(...)`, `F1Decision`.

- [ ] **Step 1: Write RED tests for split isolation and decision boundaries**

Use synthetic identity blocks and assert train/validation identities are disjoint. Spy on fitting and selection to prove official query/gallery arrays are never passed. Test exact 1.0-point quality and 0.40-point Pareto boundaries, unavailable comparator rejection, and the negative-result fidelity-only branch.
...
Use a PyTorch `nn.Linear(D, 512, bias=False)`, fixed registered optimizer grid, training-identity batches, and validation R@1 selection. Apply the identical fit/search protocol to every F0-green foundation arm and the local fallback anchor. Return `CONTINUE`, `CLOSE_FOUNDATION_TRANSFER`, or `UNAVAILABLE_COMPARATOR`; never inspect official-test metrics.
...
Run: `pytest -q tests/test_cli.py -k foundation_screen tests/test_foundation_pareto.py`
...
Run: `python -m py_compile src/sfora/foundation_pareto.py src/sfora/image_benchmark.py src/sfora/cli.py`
...
```bash
git add src/sfora/foundation_pareto.py src/sfora/image_benchmark.py src/sfora/cli.py tests/test_foundation_pareto.py tests/test_image_benchmark.py tests/test_cli.py docs/foundation_test_read_register.json
