...
- Produces: `build_cap_heads(features, labels, class_mean, *, row_norm) -> CapConstruction`.
...
Use a small FP64 analytic residual matrix and an injected `ledoit_wolf_fn`. Assert exact call arguments `assume_centered=True, block_size=1000`, FP32 normalization before FP64 accumulation, exact centered and uncentered right-hand sides, Cholesky solves, row norm, variant order, concrete float shrinkage, covariance digest, and rejection of nonfinite/asymmetric/non-positive-definite inputs.
...
Use frozen dataclasses with concrete Python scalar fields. Normalize FP32 fitting features first, then convert the normalized rows and class means to contiguous FP64 CPU arrays. Compute residuals in fitting-row order, call scikit-learn once, validate symmetry with `np.allclose(rtol=1e-12, atol=1e-14)`, use `np.linalg.cholesky`, solve with two triangular solves, normalize every nonzero row, scale to the registered norm, and cast once to contiguous FP32 on the input device. Compute the exact eight-set mismatch diagnostic and implement paired statistics, predicates, and decisions as pure functions that never import data or inspect files.
...
- Produces: `strict_json_object`, `authenticate_run`, `validate_result`, `write_result_atomic`, `run_parent_replay_preflight`, `parse_args`, and `main`.

- [ ] **Step 1: Write provenance and parser RED tests**

Freeze exact CLI flags `--unicom-checkout`, `--checkpoint`, `--dataset-root`, `--parent-result`, `--output`, and optional literal `--parent-replay-only`. Construct a temporary linear Git fixture and require parent artifact bytes/SHA, source commit ancestry, exact reviewed source-path digests, UniCOM revision, checkpoint, partition, clean detached checkout, real non-symlink inputs, real output parent, and absent destination/PID-temp before importing Torch.
...
In `tests/test_unicom_cap_f0_run_config.py`, validate a synthetic future config with exact ordered keys for spec authority, parent result authority, model/data/runtime bindings, reviewed revision, ordered source paths/digests, protocol constants, and commit-keyed result path. Require a direct config-only handoff child of a synthetic reviewed source and reject reordered/extra keys, wrong ancestry, source drift, an existing result, and an existing PID-temp.
...
Keep module-scope imports limited to the standard library and NumPy. Authenticate literal source bytes against Git blobs before importing `sfora.unicom_cap`, `sfora.unicom_probe`, Torch, or scikit-learn. Implement type/order-aware validation that recomputes all derived fields from primitive persisted rows rather than comparing a payload to itself.
...
Save/restore Python, NumPy, CPU Torch, and all CUDA RNG states around the scientific call. Pin CPU BLAS/OpenMP threads to one and require the exact runtime versions and GB10 CUDA device. Record elapsed seconds and peak allocated GPU MiB only after synchronization.
...
- [ ] **Step 4: Record the reviewed source identity**
...
Populate the already-reviewed exact schema with `V_CAP`, its ordered source digests, and output path `reports/generated/unicom-cap-f0-<V_CAP[:7]>.json`. Do not edit the validator after source review.
...
Run the frozen CLI once with exact absolute paths. Retain its PID/session, poll that same process at intervals no longer than 55 seconds, report liveness and errors, and never launch a replacement.
