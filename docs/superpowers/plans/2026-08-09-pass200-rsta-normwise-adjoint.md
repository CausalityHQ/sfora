...
- [ ] **Step 2: Freeze reviewed calibration source and run exactly once**
  ...
  verify the literal reviewed runtime and retain its path in `pinned_venv` for
  direct use by the calibration subshell. Then derive the
...
PY
  ...
  (
    ...
    exec env CUDA_VISIBLE_DEVICES='' "${pinned_venv}/bin/python" \
