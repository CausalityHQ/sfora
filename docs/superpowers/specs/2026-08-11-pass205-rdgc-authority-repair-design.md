...
`91d7d58`, chronology self-consistency fix `9b3367e`, then the review-fix commit
containing these bytes. A document cannot self-bind that last commit; the later
amendment must derive it from Git, freeze it literally, and disclose the
complete order. These reviewed design bytes postdate the corrected audit they
incorporate.
