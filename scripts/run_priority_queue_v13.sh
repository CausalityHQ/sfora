...
#
# Usage:  nohup bash scripts/run_priority_queue_v13.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v13 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v13 start"

# ---------------------------------------------------------------------------
# Ordered so that each block answers ONE question, cheapest-decisive first.
# In-Shop sigma is 0.12 pt, so a SINGLE seed already resolves the 1.39 pt effect
# H3 is about - three seeds are confirmation, not discovery. That frees the GPU
# for the novelty candidate far earlier than running all six In-Shop arms first.
# ---------------------------------------------------------------------------

# 1. Does the BatchNorm fix rescue HERD? One In-Shop seed answers it (~4.6h).
#    Baseline HERD 0.8906 / 0.8892 / 0.8900.
run_arm inshop herd_bnfix hist herd_bnfix 0

# 2. THE NOVELTY CANDIDATE. Region Proxy Anchor: an image is a SET of g x g region
#    descriptors scored against class proxies by a soft maximum over regions, rather
#    than one pooled vector. Motivated by the antihub measurement here (5-8% of CUB
#    test images in nobody's top-10, failing their own retrieval by 21 pt) on the
#    hypothesis that global pooling averages away the single locally-discriminative
#    region separating two fine-grained classes.
#    Baseline is official Proxy Anchor: 0.6825 seed 0 (CUB).
for seed in 0 1 2; do
  run_arm cub region_pa proxy_anchor region_pa "${seed}"
done

# 3. The other two untested objectives, now that run_arm selects them correctly.
for seed in 0 1 2; do
  run_arm cub local_nca hist local_nca "${seed}"
done

# 4. Confirmation seeds for H3, plus the Proxy Anchor leg.
for seed in 1 2; do
...
# 5. Remaining CUB baselines for the record.
for seed in 1 2; do
...
log "priority queue v13 complete"
