#!/usr/bin/env bash
# Frozen serial seed replication: paired control/freeze at 1,000 updates.
set -euo pipefail
...
for seed in 179024 179025; do
  for arm in control freeze; do
    test -s "$RUNS/sfora-inshop-unseen-$arm-$seed-smoke17-v2/receipt.json"
    test ! -e "$RUNS/sfora-inshop-unseen-$arm-$seed-v2"
  done
done
...
for seed in 179024 179025; do
  ...
  for arm in control freeze; do
    output="$RUNS/sfora-inshop-unseen-$arm-$seed-v2"
    echo "START unseen-gallery replication seed=$seed arm=$arm" >&2
    ...
    echo "DONE unseen-gallery replication seed=$seed arm=$arm receipt_sha256=$(sha256sum "$output/receipt.json" | cut -d' ' -f1)" >&2
  done
done
