...
#
# Usage:  nohup bash scripts/run_priority_queue_v23.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v23 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v23 start"

# ---------------------------------------------------------------------------
# CARS196 HAS NEVER BEEN RUN UNDER CORRECTED RECIPES. Zero artifacts. The entire
# digest-pinned matrix is CUB + In-Shop, and Cars is one of the three benchmarks this
# project claims to work on. That is a hole in the record independent of any method
# question.
#
# It also happens to be the one clean test of where distillation pays. The +0.658 pt
# pa_distill effect is CUB-only under official recipes. Comparing CUB with In-Shop
# cannot explain it, because those two differ in THREE ways at once: class count
# (100 vs 3997), backbone (ResNet-50 vs BN-Inception) and BatchNorm (frozen vs
# trainable). Cars holds all of that fixed against CUB - same ResNet-50, same frozen
# BatchNorm, same batch 120, same 60 epochs, 98 classes vs 100 - and changes only the
# domain. So:
#
#   replicates (~+0.5 to +0.9)  -> the effect is a property of the small-class,
#                                  frozen-BatchNorm regime rather than of CUB, and the
#                                  one surviving result becomes a two-dataset result.
#   does not replicate          -> it is CUB-specific, which after six seeds and a
#                                  refuted mechanism means it is not a finding at all.
#
# Either way this is worth more than another candidate. Note the lr schedule differs
# from CUB by design: Cars steps every 10 epochs, CUB every 5, both per the authors.
#
# Interleaved base/derived per seed so each pair completes together and an early look
# is a complete paired comparison rather than three baselines.
# ---------------------------------------------------------------------------
...
log "priority queue v23 complete"
