"""Replay an explicit packed top-k library and Python API on the frozen fixture."""
...
def main() -> None:
    ...
    if _sha256(args.api_root / "sfora/cutile_int8.py") != args.api_sha256:
        raise ValueError("Python API differs")
        ...
        with CutilePackedInt8Gallery.open(
            ...
            for batch in (1, 32):
                ...
                result: dict[str, object] = {
                    ...
                    "reference_sha256": _sha256(args.expected / f"exact_{rows}_{batch}.json"),
                }
                if rows == 1_000_000:
                    for _ in range(5):
    ...
    receipt = {
        "schema": "sfora-rc5-packed-library-replay-v1",
