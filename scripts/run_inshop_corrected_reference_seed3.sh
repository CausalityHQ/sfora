...
PROJECT_DIR="${PROJECT_DIR:-/home/riomus/sfora-inshop-seed3}"
...
REPORT_REL="reports/generated/inshop_corrected_pa_seed3.json"
CHECKPOINT_REL="reports/checkpoints/inshop_corrected_pa_seed3.pt"
...
if config["seed"] != 3 or config["recipe_id"] != "proxy_anchor.inshop.official-51db570":
    raise SystemExit("seed-3 report does not contain the locked recipe")
if checkpoint.get("artifact_selection") != "final_training_state":
    raise SystemExit("seed-3 checkpoint is not the final training state")
if checkpoint.get("training_step") != 8580:
    raise SystemExit("seed-3 checkpoint did not complete 8,580 steps")
if checkpoint.get("training_config") != config:
    raise SystemExit("seed-3 checkpoint/report configs differ")
PY
