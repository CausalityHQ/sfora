...
#
# Usage:  nohup bash scripts/run_priority_queue_v20.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v20 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v20 start"

# ---------------------------------------------------------------------------
# v19 died here at 00:52 on a selector bug of mine: `run_arm cub hist hist hist`
# passed "hist" where a base recipe needs the selector "auto" (a derived name like
# "herd" is what goes in that slot otherwise). resolve_recipe correctly rejected it,
# and `set -u` turned the empty metadata array into a fatal unbound-variable error.
# The PA leg had already finished, so no result was lost - only ~20 min of idle GPU.
#
# The PA leg came in at six paired seeds, all positive:
#   +0.91 / +1.03 / +0.73 / +0.32 / +0.86 / +0.10  ->  mean +0.658, sd 0.367
#   paired t=4.40 p_t=0.0070, exact sign p=0.031 (assumption-free)
#
# But split by whether the seed generated the hypothesis:
#   in-sample  (0,1,2): +0.91 +1.03 +0.73  ->  +0.89
#   out-of-sample (3,4,5): +0.32 +0.86 +0.10  ->  +0.43
# The effect is real and less than half the size the first three seeds implied. A
# textbook winner's curse, and the reason the confirmation seeds were worth running.
#
# This queue runs only the HIST leg, which tests the *explanation* rather than the
# effect. If "helps only an underfitting base" is right, HIST (already at CUB's
# ceiling, 0.7107) stays flat across six seeds while PA holds its +0.43. A clean
# dissociation is the result worth having; if HIST also rises, the explanation dies
# and what remains is an unexplained small gain.
# ---------------------------------------------------------------------------

for seed in 3 4 5; do
  run_arm cub hist hist auto "${seed}"
  run_arm cub herd hist herd "${seed}"
done

log "priority queue v20 complete"
