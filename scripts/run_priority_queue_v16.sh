...
#
# Usage:  nohup bash scripts/run_priority_queue_v16.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v16 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v16 start"

# 1. THE METHOD. Tversky contrast similarity replacing cosine.
#    Cognitive psychology (Tversky, Psych. Review 1977) showed human similarity is a
#    contrast of common vs distinctive features and is ASYMMETRIC - none of which
#    cosine can express. Measured here: 35-42% of CUB top-10 neighbours cross a class
#    boundary and 5-8% of images are in nobody's top-10, so the metric assumption is
#    visibly failing on this data. Differentiable Tversky exists (arXiv:2506.11035,
#    2025) but has only been evaluated on closed-set classification and language
#    modelling; this is its first use as a RETRIEVAL similarity on unseen classes.
#    Bounded ratio form, so alpha=beta=1 is exactly Tanimoto/Jaccard from chemistry.
#    Baseline: official Proxy Anchor 0.6825 (CUB seed 0). Differs only in the delta.
# 1a. Shepard's exponential kernel. Cosine-softmax is secretly Gaussian
#     (exp(cos/T) ~ exp(-d^2/2T)); Shepard derived exp(-d), linear in distance, and no
#     temperature on cosine reproduces it. Targets the orphan mechanism directly: under
#     a Gaussian kernel a moderately-distant true positive's softmax weight collapses
#     and it stops receiving gradient, which is how 5-8% of CUB images end up in
#     nobody's top-10. Cheapest possible test - only the similarity function changes.
for seed in 0 1 2; do
  run_arm cub shepard proxy_anchor shepard "${seed}"
done
run_arm cub shepard_l1 proxy_anchor shepard_l1 0

# 1b. Tversky contrast similarity, bounded ratio form.
for seed in 0 1 2; do
  run_arm cub tversky proxy_anchor tversky "${seed}"
done

# 2. Finish the H3 confirmation on the Proxy Anchor leg (herd leg already done at
#    ~20 sigma: 0.9035 / 0.9048 / 0.9041 vs HERD 0.8899).
for seed in 0 1 2; do
  run_arm inshop pa_distill_bnfix proxy_anchor pa_distill_bnfix "${seed}"
done

# 3. Remaining CUB baselines for the record.
for seed in 1 2; do
  run_arm cub proxy_anchor proxy_anchor auto       "${seed}"
  run_arm cub pa_distill   proxy_anchor pa_distill "${seed}"
done

log "priority queue v16 complete"
