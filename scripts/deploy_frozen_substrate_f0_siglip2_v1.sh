...
[[ $cell == dinov2-large || $cell == siglip2-so400m || $cell == siglip-so400m ]]
...
if [[ -n $local_error_manifest || -n $remote_error_manifest ]]; then
  ...
  case $cell in
    dinov2-large) expected_correct=1196 ;;
    siglip2-so400m) expected_correct=1227 ;;
    siglip-so400m) expected_correct=1242 ;;
...
ssh "$host" bash -s -- "$dir" "$revision" "$remote_output" "$venv" "$cell" "${remote_error_manifest:-none}" "${expected_correct:-none}" <<'REMOTE'
...
if [[ $6 != none ]]; then
  SOURCE_REVISION=$2 SOURCE_MANIFEST=$PWD/SOURCE_MANIFEST.sha256 OUTPUT=$3 CELL=$5 ERROR_MANIFEST="$6" EXPECTED_CORRECT="$7" bash scripts/run_frozen_substrate_f0_siglip2_v1.sh
else
...
if [[ -n $local_error_manifest ]]; then
  ...
  expected_manifest_sha256=$(python3 - "$local_output" <<'PY'
import json
import pathlib
import sys

print(json.loads(pathlib.Path(sys.argv[1]).read_text())["error_manifest_sha256"])
PY
)
  local_manifest_sha256=$(sha256sum "$local_error_manifest" | awk '{print $1}')
  [[ $local_manifest_sha256 == "$expected_manifest_sha256" ]]
  if [[ $cell == siglip-so400m ]]; then
