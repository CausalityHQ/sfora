...
#
# Usage:  nohup bash scripts/run_priority_queue_v11.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v11 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v11 start"

# ---------------------------------------------------------------------------
# REPRIORITISED after herd_hg_incidence came back 0.7235 / 0.7031 / 0.7070 across
# seeds 0-2: sd ~1.1 pt. CUB seed noise DWARFS every effect measured so far, and
# the +0.52 that looked like the first win over HIST was a lucky seed-0 draw.
#
# Worse, we hold only HIST seed 0, so none of those comparisons were even paired -
# we were subtracting a 3-seed mean from a single-seed baseline. Nothing else is
# interpretable until the baseline has the same seeds. So the baseline comes first,
# and no further variants are screened at n=1.
# ---------------------------------------------------------------------------

# 1. THE BASELINE. Without these, no comparison on CUB means anything.
run_arm cub  hist              hist         auto              1
run_arm cub  hist              hist         auto              2

# 1b. THE NEW OBJECTIVES, three seeds each - no more n=1 screening.
#     local_nca moves the positive sum INSIDE the log (the original NCA form), so it
#     is satisfied once some genuine same-class instance outranks the k hardest
#     cross-class ones and never drags a distant same-class sample in. Measured
#     gradient on a distant positive: 0.00004 vs SupCon's 4.99949.
#     hist_mem gives hyperedges real multi-member structure via cross-batch memory,
#     without spending the class diversity that balanced sampling cost (-2.74 pt).
#     Baseline for both is cub/hist, now at 0.7183 / 0.7010 / 0.7127 (mean 0.7107).
for seed in 0 1 2; do
  run_arm cub local_nca hist local_nca "${seed}"
done
for seed in 0 1 2; do
  run_arm cub hist_mem  hist hist_mem  "${seed}"
done

# 2. Complete the two arms that already have multi-seed data, so the paired
#    comparisons are real rather than mean-vs-single-seed.
run_arm cub  herd              hist         herd              1
run_arm cub  herd              hist         herd              2
run_arm cub  herd_hg_prototype hist         herd_hg_prototype 1
run_arm cub  herd_hg_prototype hist         herd_hg_prototype 2

# 3. Proxy Anchor baseline seeds, to test whether its apparent -1.45 vs published
#    is noise (the fidelity audit found no defect to blame).
run_arm cub  proxy_anchor      proxy_anchor auto              1
run_arm cub  proxy_anchor      proxy_anchor auto              2
run_arm cub  pa_distill        proxy_anchor pa_distill        1
run_arm cub  pa_distill        proxy_anchor pa_distill        2

# 4. H3's decisive intervention on In-Shop, where the effect (-1.39 pt) is far larger
#    than that dataset's seed noise (sd 0.0012) and therefore actually measurable.
for seed in 0 1 2; do
...
# 5. Cars replication of the corrected reference matrix.
for seed in 0 1 2; do
  run_arm cars proxy_anchor proxy_anchor auto       "${seed}"
  run_arm cars hist         hist         auto       "${seed}"
  run_arm cars pa_distill   proxy_anchor pa_distill "${seed}"
  run_arm cars herd         hist         herd       "${seed}"
done

log "priority queue v11 complete"
