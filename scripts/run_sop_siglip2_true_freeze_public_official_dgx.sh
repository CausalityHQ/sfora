...
root=/home/riomus/sfora-sop-true-freeze-public-official-v1
...
check_sha "$root/scripts/evaluate_sop_siglip2_official.py" 36192e77cfd528011e7f089518a1df46c5785fbba55b4779a47e883f59a7b61d
...
for arm in control freeze; do
  test ! -e "$runs/sfora-sop-true-freeze-public-official-$seed-$arm-v1"
done
for arm in control freeze; do
  training="$runs/sfora-sop-true-freeze-$arm-$seed-1000-v1"
  output="$runs/sfora-sop-true-freeze-public-official-$seed-$arm-v1"
  echo "START public official seed=$seed arm=$arm" >&2
  "$python" "$root/scripts/evaluate_sop_siglip2_official.py" \
    ...
    --export-batch-size 32 --inference-precision fp16_native
  echo "DONE public official seed=$seed arm=$arm receipt_sha256=$(sha256sum "$output/receipt.json" | cut -d' ' -f1)" >&2
done
