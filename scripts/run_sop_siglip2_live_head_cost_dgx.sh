...
for arm in arcface float_rank bank; do
  test -s "$run_base/sfora-siglip2-member-bank-multiseed-179022-${arm}-v1/receipt.json" || {
    echo "live-head cost screen waits for completed seed22: $arm" >&2
    exit 1
  }
done
