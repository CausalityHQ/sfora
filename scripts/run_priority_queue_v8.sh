...
#
# Usage:  nohup bash scripts/run_priority_queue_v8.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v8 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v8 start"
...
# 2b-PRIORITY. herd_hg_incidence scored 0.7235 vs HIST 0.7183 on seed 0 = +0.52 pt,
# clearing the gate. Confirming a candidate winner across seeds is worth more than
# screening further arms, so its seeds 1 and 2 jump the queue. NOTE the irony, which
# is recorded rather than smoothed over: this is the arm demoted to an ablation
# CONTROL after a test proved it is not n-ary and carries no novelty claim, while the
# provably hypergraph-native target it was meant to control for scored -0.09.
run_arm cub  herd_hg_incidence   hist herd_hg_incidence   1
run_arm cub  herd_hg_incidence   hist herd_hg_incidence   2

# Follow the evidence: extend the family that moved, not the one that was predicted
# to. herd_hg_prototype distils the same Mahalanobis affinity over ALL classes rather
# than only the ~27 present in a CUB batch of 32.
run_arm cub  herd_hg_prototype   hist herd_hg_prototype   0
...
escalate_if_promising cub herd_hg_prototype   hist herd_hg_prototype   "${HIST_CUB_BASE}"
escalate_if_promising cub herd_hg_incidence   hist herd_hg_incidence   "${HIST_CUB_BASE}"
escalate_if_promising cub herd_hg             hist herd_hg             "${HIST_CUB_BASE}"
...
log "priority queue v8 complete"
