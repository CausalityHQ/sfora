...
#
# Usage:  nohup bash scripts/run_priority_queue_v10.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v10 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v10 start"
...
log "priority queue v10 complete"
