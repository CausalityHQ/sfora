...
#
# Usage:  nohup bash scripts/run_priority_queue_v19.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v19 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v19 start"

# ---------------------------------------------------------------------------
# THE FIRST ARM TO PASS THE GATE. CUB pa_distill vs proxy_anchor, 3 seeds:
#   +0.91 / +1.03 / +0.73  ->  mean +0.89 pt, sd 0.153, paired t=10.05, p_t=0.0097
#
# Two things make this worth spending GPU on rather than filing as noise:
#
#   1. It is in the FROZEN-BatchNorm setting, so the H3 teacher/student bug is
#      provably inert here. Whatever this is, it is not the bug.
#   2. It fits the mechanism already in the record - distillation helps a base
#      that is underfitting (PA 0.6876 vs HIST 0.7107 on the same data) and does
#      nothing for one that is not (HERD-vs-HIST -0.03; In-Shop bnfix +0.03/-0.04).
#
# And one thing makes it suspect: the exact sign test floors at 0.25 for n=3, so
# assumption-free this is not yet significant, and a +0.52 pt CUB false positive
# has already burned us once at n=1. The paired sd of 0.153 is also far tighter
# than the 1.08 pt fixed-seed spread measured on HIST would predict - either PA
# runs are much more reproducible than HIST runs, or we got lucky.
#
# Three more paired seeds settle it: 6/6 positive gives an assumption-free
# p = 2^-5 = 0.031, and it independently measures the paired sd on a sample that
# is not the one that generated the hypothesis.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# The HIST leg at the same seeds. If the "helps only an underfitting base"
# reading is right, this stays flat while the PA leg keeps its ~+0.9 - and a
# clean dissociation on one dataset at six seeds is a far better result than a
# bare improvement. If instead HIST also rises, the effect is not conditional on
# headroom and the explanation is wrong.
# ---------------------------------------------------------------------------

for seed in 3 4 5; do
  run_arm cub hist hist hist "${seed}"
  run_arm cub herd hist herd "${seed}"
done

log "priority queue v19 complete"
