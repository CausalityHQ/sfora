...
different ways to shrink the pack to D dims: a GPA-aligned mean (the best fold we
found), a single-reference Procrustes mean, PCA of the concat, a random
projection, and a naive average. All of these use only the embeddings' geometry
(no labels, no retrieval targets); they are fit on the same embeddings they are
scored on, which is standard for compression but still a transductive number.
