...
#
# Usage:  nohup bash scripts/run_priority_queue_v15.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v15 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v15 start"
...
log "priority queue v15 complete"
