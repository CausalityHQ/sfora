...
#
# Usage:  nohup bash scripts/run_priority_queue_v9.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v9 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v9 start"
...
log "priority queue v9 complete"
