...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--directory',required=True); p.add_argument('--output',required=True); p.add_argument('--seeds',nargs='+',required=True); p.add_argument('--prefix',default='inshop_trained_prehead_seed'); a=p.parse_args(); rows=[]
 for s in a.seeds:
