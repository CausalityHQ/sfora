...
remote_output_root=${REMOTE_OUTPUT_ROOT:-/home/riomus/sfora-gallery-alignment}
revision=$(git rev-parse HEAD)
remote_source=$remote_root/$revision
remote_output=$remote_output_root/$revision
local_result=${LOCAL_RESULT:-/tmp/sfora-gallery-alignment-$revision.json}
local_artifact=${LOCAL_ARTIFACT:-/tmp/sfora-gallery-alignment-$revision.safetensors}
...
source_files=(
  ...
  scripts/deploy_siglip_gallery_compatibility_alignment_v1.sh
...
scratch=$(mktemp -d /tmp/sfora-gallery-alignment-deploy.XXXXXX)
cleanup() {
  case "$scratch" in
    /tmp/sfora-gallery-alignment-deploy.*) rm -rf -- "$scratch" ;;
    *) exit 99 ;;
  esac
}
trap cleanup EXIT INT TERM
bundle=$scratch/source.bundle
remote_bundle=/tmp/sfora-gallery-alignment-source-$revision.bundle
...
set -euo pipefail
pgrep -f '[p]robe_siglip_gallery_compatibility_alignment.py' >/dev/null && {
  echo 'gallery alignment process is already active' >&2; exit 75;
}
...
setsid timeout --signal=TERM --kill-after=30s 3600s \
  ...
  "$spatial_sha" "$staging/alignment.safetensors" "$staging/result.json" \
...
PY
test "$alignment" = "$staging/alignment.safetensors"
...
exec "$staging/landlock-exec" --ro /usr --ro /etc --rw /proc --ro /sys \
  ...
  --rw "$staging" -- \
  "$python" -B scripts/probe_siglip_gallery_compatibility_alignment.py \
  ...
  --alignment-artifact "$alignment" --result "$result" \
...
test -s "$staging/result.json"; test -s "$staging/alignment.safetensors"
artifact_sha=$(sha256sum "$staging/alignment.safetensors" | awk '{print $1}')
...
value = validate_alignment_result_bytes(pathlib.Path(sys.argv[1]).read_bytes())
assert value["claim_eligible"] is False
assert value["external_evaluation_access"] is False
assert value["alignment_artifact_sha256"] == sys.argv[2]
assert value["spatial_artifact_sha256"] == sys.argv[3]
payload = load_file(str(pathlib.Path(sys.argv[1]).with_name("alignment.safetensors")))
assert set(payload) == {
    "development_aligned",
    "development_student",
    "student_to_teacher",
}
matrix = payload["student_to_teacher"]
assert matrix.device.type == "cpu" and matrix.dtype == torch.float64
error = float((matrix.T @ matrix - torch.eye(matrix.shape[0], dtype=torch.float64)).abs().max())
assert error == value["orthogonality_error"]
student = payload["development_student"]
aligned = payload["development_aligned"]
assert student.device.type == aligned.device.type == "cpu"
assert student.dtype == aligned.dtype == torch.float32
assert torch.equal(apply_orthogonal_alignment(student, matrix), aligned)
score_delta = float(
    (student.double() @ student.double().T - aligned.double() @ aligned.double().T)
    .abs()
    .max()
)
assert score_delta == value["self_geometry_max_score_delta"]
PY
