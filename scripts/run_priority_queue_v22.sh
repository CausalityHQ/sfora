...
#
# Usage:  nohup bash scripts/run_priority_queue_v22.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v22 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v22 start"

# ---------------------------------------------------------------------------
# v21 refuted both explanations for the one surviving effect. Headroom varies ~5x
# across embedding widths and Delta does not track it (512: +0.66, 128: -0.08,
# 64: +0.48), which was the pre-registered falsification condition; the capacity
# rival predicted Delta(64) < -0.89 and got +0.42/+0.54.
#
# So EMA self-distillation adds ~+0.4 pt to Proxy Anchor on CUB (6/6 seeds,
# assumption-free p=0.031) with no mechanism. This queue tests the most mundane
# candidate, which is also the one nobody checks.
#
# An EMA teacher does two separable things: it supplies a distillation target, and it
# is an averaged copy of the weights. pa_distill conflates them. These arms keep the
# teacher, score retrieval FROM it, and add no loss at all.
#
#   reproduces ~+0.4  -> the distillation loss is inert and this is Polyak/SWA weight
#                        averaging under another name. A pointed finding about a whole
#                        class of reported gains, not just about us.
#   reproduces ~0     -> the loss is doing real work; still unexplained, but localised
#                        to the loss rather than to the teacher.
#
# TWO momenta, because one would give an ambiguous null. At 0.999 over CUB's 2940 steps
# the EMA retains 0.999^2940 = 5.3% of its INITIALISATION - pretrained backbone, but a
# randomly-initialised embedding head. That copy could score badly for a reason having
# nothing to do with averaging. At 0.99 the initial term is 3e-13, i.e. gone.
#
#   pa_ema_avg       0.999, matching pa_distill exactly. Isolates the teacher that arm
#                    actually distils toward, contamination included.
#   pa_ema_avg_fast  0.99. Tests weight averaging as such.
#
# Three seeds each rather than six of one. This is exploratory mechanism work, so
# finding the direction across both arms beats an assumption-free test of one; whichever
# arm moves gets its confirmation seeds afterwards. Baselines are already owned at six
# seeds (proxy_anchor 0.6919), so every run here buys a complete paired comparison.
# ---------------------------------------------------------------------------

for seed in 0 1 2; do
  run_arm cub pa_ema_avg      proxy_anchor pa_ema_avg      "${seed}"
  run_arm cub pa_ema_avg_fast proxy_anchor pa_ema_avg_fast "${seed}"
done

log "priority queue v22 complete"
