...
#
# Usage:  nohup bash scripts/run_priority_queue_v17.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v17 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v17 start"

# ---------------------------------------------------------------------------
# CUB CANNOT ADJUDICATE THESE METHODS. sigma is 0.88 pt across seeds and 1.08 pt at
# FIXED seed, so a +-1 pt result is unreadable - shepard's two seeds (0.6710, ~0.677)
# sit inside noise of PA's 0.6825 and will stay unreadable at n=3.
#
# In-Shop sigma is 0.12 pt against a PA baseline of 0.9035 (3 seeds). ONE seed there
# resolves a 0.5 pt effect. Runs cost ~2.2 h vs CUB's ~45 min, but 2 In-Shop runs buy
# a verdict that 6 CUB runs cannot. So both methods go straight to the decisive test.
# ---------------------------------------------------------------------------

# 1. Finish the CUB screen already in flight (cheap, and it establishes direction).
for seed in 0 1 2; do
  run_arm cub shepard proxy_anchor shepard "${seed}"
done

# 2. THE DECISIVE TEST. Both methods on In-Shop, one seed each, against PA 0.9035.
run_arm inshop shepard proxy_anchor shepard 0
run_arm inshop tversky proxy_anchor tversky 0

# 3. CUB tversky screen, and the city-block Shepard variant.
for seed in 0 1 2; do
  run_arm cub tversky proxy_anchor tversky "${seed}"
done
run_arm cub shepard_l1 proxy_anchor shepard_l1 0

# 4. Confirmation seeds on In-Shop for whichever direction survives, plus the
#    remaining H3 leg (pa_distill_bnfix seed 0 already landed at 0.9029).
for seed in 1 2; do
  run_arm inshop shepard proxy_anchor shepard "${seed}"
done
for seed in 1 2; do
  run_arm inshop tversky proxy_anchor tversky "${seed}"
done
for seed in 1 2; do
  run_arm inshop pa_distill_bnfix proxy_anchor pa_distill_bnfix "${seed}"
done

log "priority queue v17 complete"
