...
#
# Usage:  nohup bash scripts/run_priority_queue_v12.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v12 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v12 start"

# ---------------------------------------------------------------------------
# MOVED TO IN-SHOP. CUB cannot adjudicate anything this project is testing:
# sigma = 0.88 pt across seeds AND a 1.08 pt spread at FIXED seed (three
# effectively-identical runs scored 0.7183 / 0.7154 / 0.7075). Detecting +0.5 pt
# there needs 12-37 seeds per arm = 8-25 GPU-hours per arm.
#
# In-Shop has sigma = 0.12 pt. One seed resolves +0.5 pt; three make it certain.
# Runs cost ~3x more each but need ~15x fewer, so In-Shop is BOTH cheaper and
# conclusive. Every method question moves here.
# ---------------------------------------------------------------------------

# 1. H3's decisive intervention, robust leg first. HERD regresses -1.39 pt on
#    In-Shop (paired t=-33.9, p~0.0009) - the only method effect anywhere in this
#    project that clearly exceeds its own noise. If making the EMA teacher
#    normalisation-consistent flips that sign, the negative result was a BatchNorm
#    bug and HERD is alive. Baselines already owned:
#      HERD 0.8906 / 0.8892 / 0.8900     PA 0.9024 / 0.9048 / 0.9032
...
# 2. Only after the decisive test: finish the CUB reference matrix for the record.
#    These are baselines, not method questions, so CUB's noise floor is tolerable.
for seed in 1 2; do
  run_arm cub proxy_anchor proxy_anchor auto       "${seed}"
  run_arm cub pa_distill   proxy_anchor pa_distill "${seed}"
done

# 3. The new objectives, now that run_arm actually selects them. CUB is only a
#    smoke test here - it establishes that they train and roughly where they land,
#    NOT whether they beat anything. The verdict comes from In-Shop.
for seed in 0 1 2; do
  run_arm cub local_nca hist local_nca "${seed}"
done
run_arm cub hist_mem            hist hist_mem            0
run_arm cub hist_shot_geometric hist hist_shot_geometric 0

log "priority queue v12 complete"
