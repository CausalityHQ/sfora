...
#
# Usage:  nohup bash scripts/run_priority_queue_v25.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v25 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v25 start"

# ---------------------------------------------------------------------------
# MOMENTUM SWEEP FOR EMA EVALUATION. Runs after v24; every v24 arm is digest-cached
# and will SKIP, so this is safe to launch over the same matrix.
#
# Once the best-over-training selection bonus is removed, weight averaging is the
# strongest single intervention this project has measured:
#
#   pa_ema_avg_fast - proxy_anchor   reported +0.414   corrected +0.732
#   pa_ema_avg      - proxy_anchor   reported +0.059   corrected +0.610
#   pa_distill      - proxy_anchor   reported +0.658   corrected +0.592
#
# Arms that evaluate the averaged weights collect 2.5x to 10x less selection bonus than
# arms that evaluate the student (0.07-0.31 pt against 0.77-0.84), because averaging
# smooths the evaluated model's test curve and a max over a smooth curve overshoots less.
# The protocol pays the noisier arm more, so it hides this method.
#
# Momentum is averaging's only real hyperparameter and EMA EVALUATION has never been
# assessed on these benchmarks, so no prior work fixes it. Both sampled points sit at the
# long end (windows of 1000 and 100 steps). This adds the short end, where the average
# approaches the student and the benefit MUST vanish by construction - so the curve has
# to turn over somewhere, and the turning point is the thing worth knowing.
#
# Seed 0 of both first: two runs, ~2 h, enough to find the direction. Confirmation seeds
# follow only for whichever momentum wins, which is the screening protocol the winner's
# curse taught - a 3-seed sweep of a losing momentum is wasted GPU.
# ---------------------------------------------------------------------------
...
log "priority queue v25 complete"
