...
#
# Usage:  nohup bash scripts/run_priority_queue_v14.sh > /dev/null 2>&1 &
set -uo pipefail
...
while gpu_busy; do
  ...
  log "v14 waiting for in-flight run to finish (GPU busy)..."
  sleep 120
done

log "priority queue v14 start"

# region_pa is RE-RUN, not resumed: its earlier artifacts were scored with concat
# cosine while the loss optimised a soft max over regions, so they measure a
# train/eval mismatch rather than the method. MaxSim retrieval is now wired in.

# 1. The novelty candidate, now measured with the similarity it was trained for.
#    Baseline: official Proxy Anchor 0.6825 (CUB seed 0).
for seed in 0 1 2; do
  run_arm cub region_pa proxy_anchor region_pa "${seed}"
done

# 2. The other untested objective. Baseline: hist 0.7107 (3-seed mean).
for seed in 0 1 2; do
  run_arm cub local_nca hist local_nca "${seed}"
done

# 3. H3 confirmation. Already 0.9035 / 0.9048 vs HERD's 0.8899 mean - ~12 sigma.
run_arm inshop herd_bnfix hist herd_bnfix 2
for seed in 0 1 2; do
  run_arm inshop pa_distill_bnfix proxy_anchor pa_distill_bnfix "${seed}"
done

# 4. Remaining CUB baselines for the record.
for seed in 1 2; do
...
log "priority queue v14 complete"
