...
def _payload(*, evaluations: int = 35) -> dict[str, object]:
    recipe = reference_recipe("recall_at_k_surrogate", "cars")
    history = [0.70 + i / 10000 for i in range(evaluations)]
    history[19] = 0.807
...
def test_analyze_refuses_partial_history() -> None:
    with pytest.raises(ValueError, match="35 source-cadence evaluations"):
        _MODULE.analyze(_payload(evaluations=34))
